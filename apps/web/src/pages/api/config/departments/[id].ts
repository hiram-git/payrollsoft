...
export const POST: APIRoute = async ({ request, cookies, params, redirect }) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (msg.toLowerCase().includes('children') || res.status === 422) {
          return redirect(`/config/departments/${id}?error=has_children`)
        }
        return redirect(`/config/departments/${id}?error=server-error`)
      }
    } catch {
      return redirect(`/config/departments/${id}?error=server-error`)
    }
  ...
  if (!body.code || !body.name) return redirect(`/config/departments/${id}?error=missing-fields`)
  ...
  if (res.status === 401) return redirect('/login')
  if (res.ok) return redirect(`/config/departments/${id}?success=1`)

  const data = (await res.json().catch(() => ({}))) as { error?: string }
  const msg = data.error ?? ''

  if (msg.toLowerCase().includes('code') || res.status === 409) {
    return redirect(`/config/departments/${id}?error=code_taken`)
  }
  if (msg.toLowerCase().includes('cycle') || res.status === 422) {
    return redirect(`/config/departments/${id}?error=cycle`)
  }
  return redirect(`/config/departments/${id}?error=server-error`)
}
